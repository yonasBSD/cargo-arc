...
describe('VirtualEdgeLogic.determineAggregatedDirection', () => {
  test('all upward returns upward', () => {
    expect(
      VirtualEdgeLogic.determineAggregatedDirection(['upward', 'upward']),
    ).toBe('upward');
  });

  test('all downward returns downward', () => {
    expect(
      VirtualEdgeLogic.determineAggregatedDirection(['downward', 'downward']),
    ).toBe('downward');
  });

  test('mixed directions returns downward fallback', () => {
    expect(
      VirtualEdgeLogic.determineAggregatedDirection(['upward', 'downward']),
    ).toBe('downward');
  });
...
describe('VirtualEdgeLogic.aggregateHiddenEdges', () => {
  ...
  test('no hidden edges returns empty map', () => {
    ...
    const result = VirtualEdgeLogic.aggregateHiddenEdges(
      edges,
      mockVisibleAncestor({}),
    );
  ...
  test('from hidden, to visible: uses visible ancestor for from', () => {
    ...
    const result = VirtualEdgeLogic.aggregateHiddenEdges(
      edges,
      mockVisibleAncestor({ child: 'parent' }),
    );
  ...
  test('both hidden with same visible ancestor (self-loop) is excluded', () => {
    ...
    const result = VirtualEdgeLogic.aggregateHiddenEdges(
      edges,
      mockVisibleAncestor({ a: 'root', b: 'root' }),
    );
    expect(result.size).toBe(0);
  });
    ...
    const edges = [
      {
        ...
        fromHidden: true,
        toHidden: false,
        sourceLocations: 'Symbol1  ← file1:10',
        direction: 'downward',
      },
      {
        arcId: 'c-b',
        ...
        fromHidden: true,
        toHidden: false,
        sourceLocations: 'Symbol2  ← file2:20',
        direction: 'downward',
      },
    ];
    const result = VirtualEdgeLogic.aggregateHiddenEdges(
      edges,
      mockVisibleAncestor({ a: 'root', c: 'root' }),
    );
...
describe('VirtualEdgeLogic.prepareVirtualEdgeData', () => {
  // Mock ArcLogic
  const mockArcLogic = {
    calculateArcPath(fromX, fromY, toX, toY, maxRight, _rowHeight) {
      return {
        path: `M ${fromX},${fromY} Q ${maxRight},${(fromY + toY) / 2} ${toX},${toY}`,
        toX,
        toY,
        ctrlX: maxRight + 35,
        midY: (fromY + toY) / 2,
  ...
  test('arc path is calculated via ArcLogic', () => {
    ...
    const result = VirtualEdgeLogic.prepareVirtualEdgeData(
      virtualEdges,
      nodePositions,
      300,
      mockArcLogic,
